...
public class DefaultConverterFactory implements ConverterFactory, Reportable {
	...
	public DefaultConverterFactory() {
		this(
				HashMapUtility
						.<ConverterKey, Converter<Object, Object>> getConcurrentLinkedHashMap(CACHE_SIZE),
	...
	public synchronized Converter<Object, Object> getConverter(
		...
		Converter<Object, Object> converter = _converter(sourceClass,
				destinationClass);
		...
		if (sourceClass.isPrimitive()) {
			sourceClass = TypeFactory.valueOf(ClassUtil
					.getWrapperType(sourceClass.getRawType()));
		...
		if (destinationClass.isPrimitive()) {
			destinationClass = TypeFactory.valueOf(ClassUtil
					.getWrapperType(destinationClass.getRawType()));
			converter = _converter(sourceClass, destinationClass);
		}
		if (converter != null)
			return converter;
	...
	private Converter<Object, Object> _converter(Type<?> sourceClass,
	...
	public <S, D> void registerConverter(Converter<S, D> converter) {
		...
		if (converter instanceof BidirectionalConverter
			...
			converters
					.add((Converter<Object, Object>) ((BidirectionalConverter<?, ?>) converter)
							.reverse());
		}
	}
	...
	public void reportCurrentState(StringBuilder out) {
		...
		out.append("\nRegistered converters: ").append(converters.size())
				...
				.append(humanReadableSizeInMemory(converters)).append(")");
		int index = 0;
		for (Converter<Object, Object> converter : converters) {
		...
		out.append("\nConverter cache: ").append(converterCache.size())
				.append(" (approximate size: ")
				.append(humanReadableSizeInMemory(converterCache)).append(")");
		for (Entry<ConverterKey, Converter<Object, Object>> entry : converterCache
				.entrySet()) {
			Type<?> srcType = TypeFactory.valueOf(entry.getKey()
					.getSourceClass());
			Type<?> dstType = TypeFactory.valueOf(entry.getKey()
					.getDestinationClass());
			String srcName = TypeFactory.nameOf(srcType, dstType);
			String dstName = TypeFactory.nameOf(dstType, srcType);

			out.append("\n  [").append(srcName).append(" -> ").append(dstName)
